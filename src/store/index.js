...
const getters = {
  ...
  getTopicArticles: state => state.topicArticles,
  getBonus: state => state.texts,
  getLoginway: state => state.loginway,
  getArticleFlag: state => state.articleFlag,
...
const actions = {
  displayArticles: ({commit}, payload) => {
    commit('DISPLAY_ARTICLES', payload)
  },
  displayTopic: ({commit}, type) => {
    commit('DISPLAY_TOPIC', type)
  },
  sortContent: ({commit}, type) => {
    commit('SORT_CONTENT', type)
  },
  changeLoginway: ({commit}, loginway) => {
    commit('CHANGE_LOGINWAY', loginway)
  },
  changeArticleFlag: ({commit}, flag) => {
